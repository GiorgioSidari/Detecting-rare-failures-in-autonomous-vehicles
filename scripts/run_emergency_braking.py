#!/usr/bin/env python3
"""
Runner con output leggibile per lo scenario emergency_braking — mirror di
scripts/run_lanekeeping.py. Di default usa la modalita' video-CNN su CARLA
(Fase 2 del piano); con --scenario puoi passare alla fisica ideale o, in
aggiunta a --use-nn, allo scalar-MLP.

Uso:
    python scripts/run_emergency_braking.py                       # 20 campioni, video-CNN (CARLA)
    python scripts/run_emergency_braking.py --n 50
    python scripts/run_emergency_braking.py --scenario emergency_braking            # fisica ideale
    python scripts/run_emergency_braking.py --scenario emergency_braking --use-nn   # scalar-MLP
    python scripts/run_emergency_braking.py --n 30 --seed 7 --quiet --trace-worst

Prerequisiti (modalita' video-CNN, default):
    opensbt-core/CarlaSimulator_0916/CarlaUE4.exe -RenderOffScreen -nosound
    cd opensbt-core && uvicorn Simulator.emergency_braking.SimulatorServer:app --port 8100
Vedi README.md, sezione "Emergency Braking — how to run each mode".
"""
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Esegue la pipeline emergency_braking con output chiaro.")
    ap.add_argument("--n", type=int, default=20, help="numero di campioni LHS (default 20)")
    ap.add_argument("--workers", type=int, default=None,
                    help="numero di istanze CARLA parallele (default: EB_NUM_WORKERS env o 1)")
    ap.add_argument("--seed", type=int, default=42, help="seed (default 42)")
    ap.add_argument("--scenario", default="emergency_braking_carla",
                    help="'emergency_braking_carla' (default, video-CNN) | "
                         "'emergency_braking' (fisica ideale, o scalar-MLP con --use-nn)")
    ap.add_argument("--use-nn", action="store_true",
                    help="con --scenario emergency_braking, usa lo scalar-MLP invece della fisica ideale")
    ...
    ap.add_argument("--trace-worst", action="store_true",
                    help="stampa la traccia passo-passo (posizione, velocita') del run peggiore")
    ap.add_argument("--sampling", choices=["uniform", "realistic"], default="uniform",
                    help="'realistic' richiede che lo scenario esponga param_distributions() "
                         "(non ancora implementato per emergency_braking: ricade su 'uniform')")
    args = ap.parse_args()

    # IMPORTANTE: va impostato PRIMA di importare scenarios/pipeline, perche' il
    # pool viene costruito all'import di scenarios/.
    if args.workers is not None:
        os.environ["EB_NUM_WORKERS"] = str(args.workers)
    n_workers = int(os.environ.get("EB_NUM_WORKERS", "1"))

    import numpy as np

    if args.scenario == "emergency_braking" and args.use_nn:
        from scenarios import SCENARIOS
        from scenarios.emergency_braking.config import EmergencyBrakingScenario
        SCENARIOS["emergency_braking"] = EmergencyBrakingScenario(use_nn=True)
    ...
    if is_carla:
        print(f" Worker pool    : {n_workers}  (EB_NUM_WORKERS={n_workers})")
    print(sub)
    if not args.quiet:
    ...
    print(f"   [{vals}]")
    print("   (valori < 0 = impatto/fallimento; piu' negativo = peggiore)")
    print(sub)
    ...
    def fmt_params(row):
        ...
        return (f"speed={row[0]:.1f}m/s  friction={row[1]:.2f}  "
                f"detect_dist={row[2]:.1f}m  delay={row[3]:.2f}s")
    ...
    # classifica dal profilo di velocita' se/quando ha frenato.
    def _explain(i):
        ...
        pos, vel = traj[:L, 0], traj[:L, 1]
        row = np.asarray(r.params[i], dtype=float)
        peak_idx = int(vel.argmax())
        react_frac = peak_idx / max(L - 1, 1)
        quando = "subito" if react_frac < 0.3 else ("a meta' corsa" if react_frac < 0.7 else "tardi")
        if m[i] < 0:
            mode = (f"impatto: superato il punto critico di {abs(m[i]):.1f}m, "
                    f"velocita' residua {vel[-1]:.1f} m/s")
        else:
            mode = f"fermato con margine di {m[i]:.1f}m"
        ctx = (f"; ha iniziato a decelerare {quando} (al {react_frac * 100:.0f}% della corsa); "
               f"velocita' iniziale {row[0]:.0f}m/s, attrito {row[1]:.2f}")
        return f"{mode}{ctx}"
    ...
    if args.trace_worst:
        ...
        print("   step |  posizione  | velocita'")
        for t in range(L):
            print(f"   {t:4d} | {wtraj[t, 0]:9.3f}m | {wtraj[t, 1]:7.3f} m/s")
        print("   (velocita' che non arriva a 0 = non si e' fermato in tempo)")
        print(sub)
