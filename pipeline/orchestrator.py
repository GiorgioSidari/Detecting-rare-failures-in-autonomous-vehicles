...
def run(
    ...
    # 1. Latin Hypercube Sampling. unit_samples in [0,1]^d are stratified (good coverage). How
    #    they map to parameter space depends on the sampling mode:
    #      - "uniform"   : linear scale to the bounds (equiprobable -> measures SEVERITY)
    #      - "realistic" : ppf transform of the operational distributions (weights scenarios by
    #                      how likely they are in real driving -> the failure fraction estimates
    #                      P(failure) under the ODD)
    d = len(bounds["lower"])
    ...
    # 6. Rare failure detection (SEVERITY: the worst k% by margin).
    ...
    # 6b. RARITY: failure-probability estimate. Under "realistic" sampling the samples are drawn
    #     (via ppf) from the operational distribution, so the failure fraction over valid runs is
    #     an estimate of P(failure) under the ODD. Under "uniform" it is P(failure) over a uniform
    #     ODD (baseline). A Wilson CI (robust for p near 0/1 and small n) attaches uncertainty.
    #     Rarity (probability) and severity (bottom-k%) stay DISTINCT.
