...
"""
Trains the emergency_braking CNN on the CARLA dataset collected by
collect_carla_dataset.py — the video-CNN replacement for the scalar-feature
BrakingMLP (nn_controller.py), same behavioral-cloning philosophy as
lanekeeping (simulators/common/pilotnet.py + camera_dataset.py).

Usage:
    python -m scenarios.emergency_braking.train_cnn
    python -m scenarios.emergency_braking.train_cnn --epochs 40 --batch-size 32
"""
...
MODEL_NAME = "emergency_braking_cnn"
# train_pilotnet's ModelCheckpoint writes a timestamped file per run (history,
# like lanekeeping's convention); CnnAgent/SimulatorServer need a stable path,
# so we also save a copy here after training.
STABLE_MODEL_PATH = os.path.join(MODELS_DIR, f"{MODEL_NAME}.h5")

# Camera was captured at 320x160 (see carla_episode.CAMERA_WIDTH/HEIGHT). Crop off
# the top (sky) — no dashboard/hood is visible at this mount height/pitch — then
# resize to the classic PilotNet input size.
CARLA_CROP_TOP = 40
TARGET_WIDTH = 200
TARGET_HEIGHT = 66
...
class _BrakingDataGenerator(CameraDataGenerator):
    def __init__(self, X, y, batch_size: int = 64, is_training: bool = True):
        super().__init__(
            ...
            # braking_force doesn't depend on left/right — unlike steering, flip/
            # translate augmentation must NOT flip the sign of the label (see
            # simulators/common/camera_dataset.py::random_flip).
...
def train(epochs: int = 30, batch_size: int = 64, seed: int = 42, archive_path: str = ARCHIVE_PATH) -> None:
    if not os.path.exists(archive_path):
        raise FileNotFoundError(
            ...
            "Esegui prima: python -m scenarios.emergency_braking.collect_carla_dataset"
        )
    ...
    model, history = train_pilotnet(
        ...
        input_shape=INPUT_SHAPE,
        output_dim=1,
        data_generator_cls=_BrakingDataGenerator,
    ...
    # `model` holds the best-epoch weights (EarlyStopping restore_best_weights=True).
    model.save(STABLE_MODEL_PATH)
    print(f"Modello salvato (path stabile) → {STABLE_MODEL_PATH}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Allena la CNN emergency_braking sul dataset CARLA.")
