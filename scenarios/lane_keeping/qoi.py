...
def composite_lane_qoi(
    ...
    *,
    ...
    # Validity mask - True for real steps, False for padding. run_lengths may carry an extra
    # leading row (a 'nominal' sample dropped by the orchestrator): align to the tail.
    ...
    # Degenerate/aborted runs: too few steps to be real driving (the QoI would wrongly reward them).
    survival = np.asarray(run_lengths, dtype=float)
    degenerate = survival < MIN_VALID_STEPS

    # Incoherent params: min_speed > max_speed (cols 5, 6).
    if params.shape[1] > 6:
        bad_params = np.asarray(params)[:, 5] > np.asarray(params)[:, 6]
    else:
        bad_params = np.zeros(len(survival), dtype=bool)

    # Control-loop fidelity: under-sampled runs are excluded so rates don't depend on worker count.
    def _tail(arr):
        if arr is None:
            return None
    ...
    control_hz = _tail(control_hz)
    meters_per_step = _tail(meters_per_step)
    infer_ms = _tail(infer_ms)
    wait_ms = _tail(wait_ms)
